...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (!isImage && !isVideo) {
      return new NextResponse("Unsupported file type. Use JPG, PNG, WEBP, GIF, MP4, MOV, or WEBM.", { status: 400 });
    }
    const sizeMb = file.size / (1024 * 1024);
    if (sizeMb > MAX_FILE_MB) {
      return new NextResponse(`File too large. Max ${MAX_FILE_MB}MB.`, { status: 400 });
    }
